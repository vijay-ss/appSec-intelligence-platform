...
def seed_registry():
    ...
    conn.commit()
    conn.close()
    print(f"[registry] seeded {len(SERVICES)} services")
...
class ScenarioHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/scenario":
            ...
            print(f"[scenario] fired: {scenario_name} ({len(events)} events)")
...
def main():
    ...
    if args.seed_only or args.mode == "seed-only":
        print("[generator] registry seeded — exiting")
        return
    ...
    def make_handler(*a, **kw):
        ScenarioHandler(producer, *a, **kw)

    server = HTTPServer(("0.0.0.0", 8090), lambda *a, **kw: ScenarioHandler(producer, *a, **kw))
    threading.Thread(target=server.serve_forever, daemon=True).start()
    print("[generator] scenario server running on :8090")

    # Run background hum in main thread.
    print(f"[generator] background hum at {HUM_RATE} events/second")
    hum(producer)
